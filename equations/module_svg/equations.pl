...
# 		print "variable known";
			...
			m# ^ [a-zA-Z][a-zA-Z0-9]* \s* '* \s* \( \s* [a-zA-Z][a-zA-Z0-9]* (?: \s* , \s* [a-zA-Z][a-zA-Z]* )* \s* \) \s*
                  = \s* [a-zA-Z0-9\-+*/^()., ]+ $
			   | ^ \s* [a-zA-Z0-9\-+*/^()., ]+ \s* = 
               \s* [a-zA-Z][a-zA-Z0-9]* \s* '* \s* \( \s* [a-zA-Z][a-zA-Z0-9]* (?: \s* , \s* [a-zA-Z][a-zA-Z]* )* \s* \) \s* $#x)
	{
		push @functions_strings, $assertion;
# 		print "function string";
	}

	elsif ($assertion =~ m/=/)		# EQUATIONS
	{
		push @equations_strings, $assertion;
# 		print "equation \"$assertion\"";
	}

	elsif ($assertion =~ m/^[a-zA-Z][a-zA-Z0-9]*$/)		# VARIABLES WANTED
	{
		push @var_wants, $assertion;
# 		print "variable wanted";
	}

	elsif ($assertion =~ m/^[a-zA-Z][a-zA-Z0-9]* \s* '* \s* \( \s* [a-zA-Z][a-zA-Z0-9]* (?:,\s* [a-zA-Z][a-zA-Z]* )* \s* \)$/x)
	{
		push @deriv_wants, $assertion;
# 		print "deriv wanted";
...
# sub make_closure { my $id=0; sub { $_[0]->{id} = $id++ }  }
# my $closure = make_closure();
