...
# 		print "ID $id";
		...
		push @stack, { $node->%*, side => $op_side };

		if ($node->{type} eq "binary_op") {
			($res, @path) = tree_walk_path($node->{left}, $id, "left", @stack);
			if ($res) { return 1, @path }

			($res, @path) = tree_walk_path($node->{right}, $id, "right", @stack);
			if ($res) { return 1, @path }
		}
		elsif ($node->{type} eq "unary_op") {
			($res, @path) = tree_walk_path($node->{operand}, $id, "", @stack);
			if ($res) { return 1, @path }
		}
	}
...
# 		print "ID $id";
		...
		if ($node->{type} eq "binary_op") {
			($res, @path) = tree_walk_path2($node->{left}, $var, "left", @stack);
			if ($res) { return 1, @path }

			($res, @path) = tree_walk_path2($node->{right}, $var, "right", @stack);
			if ($res) { return 1, @path }
		}
		elsif ($node->{type} eq "unary_op") {
			($res, @path) = tree_walk_path2($node->{operand}, $var, "", @stack);
			if ($res) { return 1, @path }
		}
	}
