...
if (@ARGV) {
	...
	if ($ARGV[0] eq "-i") {
		...
		if ($color) {
			shift;
			if ($ARGV[0] =~ /^\-{0,2}or|^\-{0,2}any/) {			# multiple patterns one have to match
				shift;
				print _grep4(\@ARGV, 1, "or", @keys_and_values);
			}
			elsif ($ARGV[0] =~ /^-{0,2}and|^-{0,2}all/) {		# multiple patterns all have to match
				shift;
				print _grep4(\@ARGV, 1, "and", @keys_and_values);
			}
			else {
				print _grep4(\@ARGV, 1, "and", @keys_and_values);
			}
...
# 			}
	...
	# CASE SENSITIVE
		...
		if ($color) {
			if ($ARGV[0] =~ /^\-{0,2}or|^\-{0,2}any/) {			# multiple patterns one have to match
				shift;
				print _grep4(\@ARGV, 0, "or", @keys_and_values);
			}
			elsif ($ARGV[0] =~ /^-{0,2}and|^-{0,2}all/) {		# multiple patterns all have to match
				shift;
				print _grep4(\@ARGV, 0, "and", @keys_and_values);
			}
			else {
				print _grep4(\@ARGV, 0, "and", @keys_and_values);
			}
