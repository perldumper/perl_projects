...
sub _grep2 {
	...
	while ($path = shift @_) {
		...
		for ( $j=0; $j < $path->@*; $j++) {
			...
			@pos = ();
			if ($case_insensitive) {
				while ($key =~ /($pattern)/ig) {			# find positions of all matches in a line
					push @pos, [$-[0], $+[0]];				# and foreach, push start and end positions in the line
				}
			}
			else {
				while ($key =~ /($pattern)/g) {				# find positions of all matches in a line
					push @pos, [$-[0], $+[0]];				# and foreach, push start and end positions in the line
				}
			}
...
# 						substr $flip_flop_pat, $-[0], $+[0] - $-[0], "1" x ($+[0] - $-[0]);
				...
				foreach $pat ($patterns->@*) {
					@pos_pat = ();
					$flip_flop_pat = "0" x length $key;
					while ($key =~ /($pat)/g) {		# find positions of all matches in a line
						push @pos_pat, [$-[0], $+[0]];			# and foreach, push start and end positions in the line
# 						substr $flip_flop_pat, $-[0], $+[0] - $-[0], "1" x ($+[0] - $-[0]);
					}
					foreach (@pos_pat) {
...
if (@ARGV) {
	...
	if ($ARGV[0] eq "-i") {
		if ($color) {
			# one pattern
# 			print _grep2($ARGV[1], 1, @keys_and_values);

			# multiple patterns one have to match
# 			shift;
# 			print _grep3(\@ARGV, 1, @keys_and_values);

			# multiple patterns all have to match
			shift;
			print _grep4(\@ARGV, 1, @keys_and_values);
		}
		else {
			#print grep { /$ARGV[1]/i } @output;

			# the one and only pattern have to match
# 			for (my $i=0; $i < @output; $i++) {
# 				foreach ($keys_and_values[$i]->@*) {
# 					if ( $_->{value} =~ m/$ARGV[1]/i ) {
# 						print $output[$i];
# 						next;
# 					}
# 				}
...
# 			}
	...
	# CASE SENSITIVE
		...
		if ($color) {
			# one pattern
# 			print _grep2($ARGV[0], 0, @keys_and_values);
			
			# multiple patterns one have to match
# 			print _grep3(\@ARGV, 0, @keys_and_values);

			# multiple patterns all have to match
			print _grep4(\@ARGV, 0, @keys_and_values);
		}
		else {
