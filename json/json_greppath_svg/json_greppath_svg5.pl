...
use strict;
# use warnings;	# sub _grep() -> Use of uninitialized value in print at ./json_greppath.pl line 451, <> chunk 1
use utf8;		# required ?
...
sub _grep2 {
	...
	while ($path = shift @_) {
		...
		for ( $j=0; $j < $path->@*; $j++) {
			...
			if ($path->[$j]->{type} eq "string" and $j < $path->$#* ) {
				$line .= "{\""
			}
			elsif ($path->[$j]->{type} eq "string" and $j == $path->$#* ) {
				$line .= "\""
			}
			elsif ($path->[$j]->{type} eq "index") {
				$line .= "["
			}
