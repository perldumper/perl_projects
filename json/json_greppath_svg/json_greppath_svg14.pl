...
my @path;
my @output;				# what is printed in colorless output
...
sub print_grep_color {
	...
	while ($path = shift @_) {
		...
		for ( $j=0; $j < $path->@*; $j++) {
			...
			else { $line .= " --> " }										# terminal value that is not a string
...
print_grep_color(\@ARGV, $color, $case_insensitive, $not, $logic, @keys_and_values);
