...
use constant _index       => 11;
use constant empty        => 12;
use constant object       => 13;
use constant array        => 14;
use constant object_comma => 15;
use constant array_comma  => 16;
use constant end          => 17;
use constant object_value => 18;
use constant array_value  => 19;
use constant malformed    => 20;
use constant root         => 21;
use constant key          => 22;
...
# 	print "SUB object_opening\n";
	if (@container_stack) {
# 		print "PUSH\n";
		push @stack, {type => _index, value => $indexes[-1]} if $container_stack[-1] eq "array";
	}
	push @container_stack, object;

	if ($tokens[$i+1]->{type} == curly_close) {  # empty object
...
# 	print "SUB object_closing\n";
	if (@container_stack >= 2) {
		$indexes[-1]++ if $container_stack[-2] == array;
	}
	pop @container_stack;
...
# 	print "SUB array_opening\n";
	if (@container_stack) {
		push @stack, {type => _index, value => $indexes[-1]} if $container_stack[-1] == array;
	}
	...
	if ($tokens[$i+1]->{type} == square_close) {  # empty array
		push @paths, [ @stack, { type => empty, value => "[]" } ];
	}
};
...
# 	print "SUB object_value\n";
	...
	if (@container_stack) {
		if ($container_stack[-1] == array) {
			push @stack, {type => _index, value => $indexes[-1]};
			$indexes[-1]++;
		}
	}
...
# 		$token = array_comma;
# 	}
# 	elsif ($container_stack[-1] == object) {
# 		$token = object_comma;
# 	}
# };
...
# 		if (grep {/$tok->[0]/}
#             grep {$state_transition{$state}->{$_} == malformed}
