...
# perl -Mdd -le 'use JSON; $file=<>;$text=decode_json($file); dd $text' raoult.json | ./datadumper_greppath_dfa.pl
...
#                              [
#                                'comment',
#                                ' saved from url=(0036)https://tools.suckless.org/farbfeld/ ',
#                                $VAR1->{'tree'}
#                              ],
#                              [
#                                'text',
...
# ->{"tree"}->[1]->[2]->[0]->"text"
# ->{"tree"}->[1]->[2]->[1]->"\n"
# ->{"tree"}->[1]->[2]->[2]->$VAR1->{'tree'}
# ->{"tree"}->[1]->[2]->[3]->[0]->"comment"
# ->{"tree"}->[1]->[2]->[3]->[1]->" saved from url=(0036)https://tools.suckless.org/farbfeld/ "
# ->{"tree"}->[1]->[2]->[3]->[2]->$VAR1->{'tree'}
# ->{"tree"}->[1]->[2]->[3]->[4]->[0]->"text"
# ->{"tree"}->[1]->[2]->[3]->[4]->[1]->"\n"
# ->{"tree"}->[1]->[2]->[3]->[4]->[2]->$VAR1->{'tree'}
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[0]->"tag"
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[1]->"html"
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[2]->{}
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[3]->$VAR1->{'tree'}
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[4]->[0]->"tag"
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[4]->[1]->"head"
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[4]->[2]->{}
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[4]->[3]->$VAR1->{'tree'}[5]
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[4]->[4]->[0]->"tag"
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[4]->[4]->[1]->"meta"
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[4]->[4]->[2]->{"content"}->"text/html; charset=UTF-8"
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[4]->[4]->[2]->{"http-equiv"}->"Content-Type"
# ->{"tree"}->[1]->[2]->[3]->[4]->[5]->[4]->[4]->[3]->$VAR1->{'tree'}[5][4]
# 
# 
...
# 		push @tokens, { type => "string", value => $1 }
		# NEED IT ???
		push @tokens, { type => "string", value => $1 =~ s/\n/\\n/rg =~ s/\r/\\r/rg =~ s/\t/\\t/rg }
	}
...
# do{\(my $o = 5)}
              ...
              (?| 
				...
				| (.*?)
					...
					\s*+  , \s*+ ' (?: (?:[^\\']|\\.)*+ ) ' \s* \)
			/gcx) {
			pop @tokens;	# remove { type => "bless", value => "bless(" }  
			push @tokens, { type => "string", value => $1 };
# 			print "DO MATCH";
		}
...
# 		push  { type => "empty", value => "{}" } on the @stack before printing (maybe) a "path" ?
# 		if yes, need to pop the stack somewhere to remove it
	}
};
...
# 	if (@container_stack == 0) {
	...
	dd \@container_stack;
	exit;
# 	exit if $tokens[$i+1]->{value} eq "text";
};
...
# 
# 	print "CALLING $state\n";
# 	print "%state_transition_subroutine KEY $state\n";
...
# PREPARE THE PATH INFORMATION SO THAT IT IS SUITABLE BOTH FOR SEARCHING AND FOR PRETTY PRINTED OUTPUT
	...
	for (my $i=0; $i < $#stack; $i++) {		# processing terminal value below
		if ( $_->{type} eq "key") {					# STRING OF A KEY
			push @keys_and_values,  $stack[$i];
		}
		elsif ( $_->{type} eq "index") {			# INDEX
			push @keys_and_values,  $stack[$i];
		}
	...
	push @keys_and_values,  $stack[-1];  # STRING TERMINAL VALUE
...
# 				$line .= "\""
		...
		if ($logic eq "or") {
			if ($not) {
				# print if none of the patterns has matched
				foreach (values $have_pattern_matched->%*) {
					if ($_ == 1) {
						return;
					}
				}
				...
				# print if one of the patterns has matched
				foreach (values $have_pattern_matched->%*) {
					if ($_ == 1) {
						print $line . "\n";
						last;
			...
			if ($not) {
				# print if one of the patterns has not matched
				foreach (values $have_pattern_matched->%*) {
					if ($_ == 0) {
						print $line . "\n";
						last;
				...
				# print if all the patterns has matched
				foreach (values $have_pattern_matched->%*) {
					if ($_ == 0) {
						return;
					}
