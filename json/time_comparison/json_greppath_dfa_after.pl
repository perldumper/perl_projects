...
# 	$container_stack[-1] --> limit cas when the syntax error is that there is something after a valid JSON object
	# so the @container_stack is empty
...
# 	print STDERR " Unexpcted token of type ($tokens[$j]->{type}) after token ($state) inside ($container_stack[-1])\n";
	print STDERR " Unexpcted token of type ($tokens[$j]->{type}) after token ($state) inside ()\n";
...
sub match_substring_positions {
	my $patterns = shift;
	my $key = shift;
	my $have_pattern_matched = shift;
...
# 	my $case_insensitive = shift;	# case insensitive or not
	...
	while ($path = shift $json_paths_to_grep->@*) {
		...
		for ($j=0; $j < $path->@*; $j++) {
			...
			@pos = match_substring_positions($patterns, $key, $have_pattern_matched);
