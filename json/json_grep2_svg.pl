...
# 		if ($stack[-1]->{type} eq $opening_token) {
		if ($stack[$i]->{type} eq $opening_token) {
			$i--;
		}
			...
			for (my $j=0; $j < @stack; $j++) {
				print " $j $stack[$j]->{type}\t$stack[$j]->{value}\n";
			}
			exit;
		}
	}
...
sub delete_value {
	...
	if ($value_type eq "array") {
		...
		if ($stack[-1]->{type} eq "string") {
			...
			print "delete_value() 1 -> string value not found\n";
			exit;
		}
		if ($container eq "object") {
			if ($stack[-1]->{type} eq "colon") {
				...
				print "delete_value() 2 -> value doesn't have a colon before, in an object\n";
				exit;
			}
			if ($stack[-1]->{type} eq "string") {
				pop @stack;									# delete key
				...
				print "delete_value() 3 -> value doesn't have a key before, in an object\n";
				exit;
			}
		}
		return;
		...
		if ($stack[-1]->{type} eq $value_type) {
			...
			print "delete_value() 4 -> $value_type not found\n";
			exit;
		}
		if ($container eq "object") {
			if ($stack[-1]->{type} eq "colon") {
				...
				print "delete_value() 5 -> value doesn't have a colon before, in an object\n";
				exit;
			}
			if ($stack[-1]->{type} eq "string") {
				pop @stack;									# delete key
				...
				print "delete_value() 6 -> value doesn't have a key before, in an object\n";
				exit;
			}
		}
		return;
	...
	if ($stack[-1]->{type} eq $opening_token) {
		...
		print "delete_value() 7 -> could not found $opening_token\n";
		exit;
	}
	...
	# if it is the value of a key/value pair, just delete the key/value pair
		if ($stack[-1]->{type} eq "colon") {
			...
			print "delete_value() 8 -> value doesn't have a colon before\n";
			exit;
		}
		if ($stack[-1]->{type} eq "string") {
			pop @stack;									# delete key
			...
			print "delete_value() 9 -> colon value doesn't have a string before\n";
			exit;
		}
		if ($stack[-1]->{type} eq "index") {
			pop @stack;
...
# 		push @stack, {type => "index", value => $indexes[-1]} if @indexes;
# 		if (@stack and @indexes) {
		if (@stack) {
...
# 			push @paths, [@stack[0..$#stack-1], {type => "index", value => $indexes[-1] }, $stack[-1]];
			$indexes[-1]++;
		}
# 		else {
# 			push @paths, [@stack];
...
# 	print " = ", $branch->[-1]->{value}, "\n";
	print "  ", $branch->[-1]->{value}, "\n";
# 	print map { s/\[[-\d>,\]]*[->,\]](\d+)\]/[$1]/gr } join("->", @path), "\n";
}
...
temporary hack
./json_tokenizer.pl avril.json  | ./json_grep2.pl | perl -pe 's/\[[-\d>,\]]*(\d)\]/[$1]/g' | less
