...
my $regex = qr% (?{print "regex"}) (?(DEFINE) (?<whitespace>[ \n\r\t]*) (?<string>(\"	(?:			[^"\\] |   (?: \\["\\/bfnrt] | \\u[0-9a-fA-F]{4} ))* \")) (?<number>(  [-]?	(?: 0 | [1-9] [0-9]* ) (?: \. [0-9]+  )?  (?: [eE] [-+]? [0-9]+ )?)) (?<TOP> (?&object) | (?&array) ) (?<array> \[ (?:  (?&whitespace) | (?&value) (?: , (?&value) )* ) \] ) (?<value> (?&whitespace)  (?: (?&string) | (?&number) | (?&object) | (?&array) | true | false | null ) (?&whitespace) ) (?<object> \{ (?:  (?&whitespace) | (?&whitespace) (?&string) (?&whitespace) : (?&value)  (? ,  (?&whitespace) (?&string) (?&whitespace) : (?&value) )* ) \}  )) ^   (?&TOP)   $ %xx;
